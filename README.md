...
*   **Compliance Posture Management**: Operational readiness audit secureops backing standards like ISO/IEC 27001:2022, SOC 2 Type II, and PCI DSS v4.0.
...
*   **AI Chatbot Assistant**: Interactive sidebar assistant that analyzes natural queries (e.g., "how do I add a new asset?") and guides users through secureops views.
...
| `/api/secureops/stats` | `GET` | Authenticated | Aggregates scorecard metrics. |
| `/api/secureops/incidents/status`| `GET` | Authenticated | Aggregates status counts (open/investigating/etc.).|
| `/api/secureops/incidents/severity`| `GET` | Authenticated | Aggregates severity counts (critical/high/etc.). |
| `/api/secureops/incidents/trend` | `GET` | Authenticated | Fetches incident trends tracking points. |
| `/api/secureops/incidents/recent`| `GET` | Authenticated | Fetches recent incident logs. |
| `/api/secureops/alerts/recent` | `GET` | Authenticated | Fetches recent real-time system alerts. |
| `/api/secureops/audit-logs/recent`| `GET` | Authenticated | Fetches recent audit trails. |
| `/api/secureops/user` | `GET` | Authenticated | Returns current session user DTO. |
